...
from . import ipcam_api_foscamhd, ipcam_api_foscamsd, ipcam_api_generic
FOSCAM_HD = 0
FOSCAM_SD = 1
...
class CameraAPIWrapper(object):
    ...
    def __init__(self, camera_number, daemon=False, verbose=True):
        ...
        if self.camera_type == FOSCAM_HD:
            self.camera = ipcam_api_foscamhd.FoscamCamera(
                camera_settings, daemon, verbose)

        elif self.camera_type == FOSCAM_SD:
            self.camera = ipcam_api_foscamsd.FoscamCamera(
                camera_settings, daemon, verbose)

        elif self.camera_type == FOSCAM_HD_OVERRIDE:
            self.camera = ipcam_api_foscamhd.FoscamCameraOverride(
                camera_settings, daemon, verbose)

        elif self.camera_type == GENERIC_IPCAM:
            self.camera = ipcam_api_generic.GenericIPCam(
                camera_settings, daemon, verbose)
    ...
    def getCameraSettings(self):
        ...
        if self.camera_type != GENERIC_IPCAM:
            ...
            if invalid:
                utils.log(
                    3, 'SETTINGS :: Camera %s - Invalid character in password: %s' %
                    (self.camera_number, invalid))
                password = ''
    ...
    def Connected(self, monitor=None, useCache=True):
        ...
        if monitor:
            if useCache:
                utils.log(
                    2, 'Camera %s :: Checking previous camera test connection result...' %
                    self.camera_number)
                ...
                if self.camera_type != GENERIC_IPCAM:
                    utils.log(
                        2, 'Camera %s :: Testing network connection to camera...' %
                        self.camera_number)
                    ...
                    utils.log(
                        2, 'Camera %s :: Connection successful.' %
                        self.camera_number)

                    # MJPEG Enable - for Service Run.  Ensures MJPEG URLs are
                    # Successful  MAYBE MOVE THIS LATER SOMEWHERE???
                    if settings.getSetting_int(
                            'stream', self.camera_number) == 1 or settings.getSetting_int(
                            'allstream', self.camera_number) != 1 or settings.getSetting_int(
                            'preview_stream', self.camera_number) != 1:
                        self.enable_mjpeg()
    ...
    def ptz_get_sensitivity(self):
        if self.camera_type == FOSCAM_HD or self.camera_type == FOSCAM_HD_OVERRIDE:
            return old_div(
                settings.getSetting_float(
                    'ptz_hd_sensitivity%s' %
                    self.camera_number), 10)
        elif self.camera_type == FOSCAM_SD:
            return self.camera.ptz_set_sensitivity(
                settings.getSetting(
                    'ptz_sd_sensitivity%s' %
                    self.camera_number))
    ...
    def getUrl(self, source, stream_type):
        ...
        if (source != SINGLE_CAMERA_PLAYER and stream_type != 1) or (
                source == SINGLE_CAMERA_PLAYER and stream_type == 1):
            return self.camera.mjpeg_url
    ...
    def getStreamType(self, source):
        ...
        if source == SINGLE_CAMERA_PLAYER:
            ...
            return settings.getSetting_int(
                'preview_stream', self.camera_number)
    ...
    def resetLocation(self):
        if settings.getSetting_int('ptz', self.camera_number) > 0:
            ...
            if reset_mode > 0:
                ...
                utils.log(
                    2, 'Camera %s :: Resetting to the home location' %
                    self.camera_number)

    def getTriggerInterval(self, motion_enabled, sound_enabled):
        """ Gets the alarm trigger interval from the camera """
        trigger_interval = settings.getSetting_int(
            'interval', self.camera_number)

        if self.camera_type != FOSCAM_SD and \
           self.camera_type != GENERIC_IPCAM:
            try:
                motion_trigger_interval = int(
                    self.camera.get_motion_detect_config()[1]['triggerInterval'])
                sound_trigger_interval = int(
                    self.camera.get_sound_detect_config()[1]['triggerInterval'])

                if motion_enabled and sound_enabled:
                    trigger_interval = min(
                        motion_trigger_interval, sound_trigger_interval)
                elif motion_enabled:

...
class FoscamCamera(object):
    ...
    @property
    def video_url(self):
        _videoUrl = "http://{0}/videostream.asf?user={1}&pwd={2}&resolution=32&rate=0".format(
            self.url, self.usr, self.pwd)
        return _videoUrl

    @property
    def mjpeg_url(self):
        _mjpegUrl = "http://{0}/videostream.cgi?user={1}&pwd={2}&resolution=32&rate=0".format(
            self.url, self.usr, self.pwd)
        return _mjpegUrl  # MJPEG stream is VGA resolution @ 15fps

    @property
    def snapshot_url(self):
        _snapshotUrl = "http://{0}/snapshot.cgi?user={1}&pwd={2}".format(
            self.url, self.usr, self.pwd)
        return _snapshotUrl
    ...
    def send_command(self, cmd, params=None):
        ...
        cmdurl = 'http://%s/%s.cgi?user=%s&pwd=%s%s' % (
            self.url, cmd, self.usr, self.pwd, paramstr)
    ...
    def get_camera_params(self, callback=None):
        ...
        return self.execute_command(
            'get_camera_params', None, callback=callback)
    ...
    def set_camera_params(self, param, value, callback=None):
        ...
        params = {'value': value,
                  'param': param}
        return self.execute_command(
            'camera_control', params, callback=callback)
    ...
    def decoder_control(self, command, onestep=None, degree=None, callback=None):  # NEW FUNCTION ###
        ...
        if onestep is not None:
            ...
            if degree is not None:
                ...
                params['degree'] = 3
        return self.execute_command(
            'decoder_control', params, callback=callback)
